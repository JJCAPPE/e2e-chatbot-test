# Databricks E2E Chatbot Application - Context for Codex
...
**IMPORTANT - For Codex**: Before running `quickstart.sh`, ask the user:

1. **"Do you have a Databricks serving endpoint ready?"**
   - If yes: Get the endpoint name
   - If no: Suggest they create one first or use the default `databricks-Codex-sonnet-4`
...
**Note for Codex**: This file is automatically loaded as context. When working on this project, refer to these guidelines for commands, patterns, and conventions. Keep this file updated as the project evolves.
